...
export default {
  ...
  watch: {
    'position': function(newVal) {
      const position = this.positionList.find(position => position.name === newVal);
      this.input.position_id = position.id;
    },
    'unit': function(newVal) {
      const unit = this.unitList.find(unit => unit.name === newVal);
  ...
  methods: {
    ...
    save(){
      axios.post(`${import.meta.env.VITE_LIVE_URL}/api/employee`, {
      ...
      .catch((err) => {
        this.$swal({
          ...
          position: "top-end",
          icon: "error",
          title: err.response.data.message || 'Failed to add employee',
          timer: 1000,
          showConfirmButton: false,
        })
      })
